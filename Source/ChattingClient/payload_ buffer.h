...
public:
	...
	size_t read(void* dest, size_t size)
	{
		...
		std::memmove(dest, &this->buffer[this->read_head], fit_size);
	...
	size_t write(void* src, size_t size)
	{
		...
		std::memmove(&this->buffer[this->write_head], src, size);
	...
	void rewind()
	{
		...
		// 정리.
		std::memmove(this->buffer, &this->buffer[this->read_head], size);
